...
fun <T> isFeatureInstalled(): Boolean = true

inline fun <reified T : Feature<D>, D : FeatureDependencies> getFeature(dependencies: D): T? = when (isFeatureInstalled<T>()) {
	true -> {
		val serviceIterator = ServiceLoader
			.load(T::class.java, T::class.java.classLoader)
			.iterator()
...
inline fun <reified T : Feature<D>, D : FeatureDependencies> requireFeature(dependencies: D): T =
	getFeature(dependencies) ?: throw IllegalStateException("Feature ${T::class.java} must be part of the code base")
